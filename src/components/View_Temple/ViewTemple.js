...
function ViewTemple() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get("http://localhost:4000/temples");
      ...
      {templeList.map((temple, index) => (
        <div key={index} className="temple-card">
          ...
          <div className="temple-card-details">
            ...
            <p className="temple-card-text">District - {temple.district ? temple.state : "Not Provided"}</p>
            <Link to={`/temple-details/${temple.id}`} className="temple-card-link">
              Know More...
            </Link>
          </div>
        </div>
